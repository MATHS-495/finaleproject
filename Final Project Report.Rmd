...
>>>>>>> ebee208ed5dfec8d2470c29b4f79c60afeb97065
[2] "age"  
...
```{r}
ggplot(data=Insurance_Claims,aes(x=factor(umbrella_limit),y=total_claim_amount))+
  geom_col(size=1,color="darkblue",width =.4)
``` 
...
```{r}
ggplot(data=Insurance_Claims,aes(x=factor(insured_education_level),y=total_claim_amount))+
  geom_col(size=1,color="darkblue",width =.4)
```
...
When we move forward and do the models we need to facotur out those variables.
```{r}
Insurance_Claims <- subset(Insurance_Claims, select = -c(policy_number, policy_bind_date, policy_csl, insured_zip, incident_date, incident_location, police_report_available, injury_claim, property_claim, vehicle_claim))
Insurance_Claims
```
